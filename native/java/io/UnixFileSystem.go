...
//对应 private native String canonicalize0(String path) throws IOException;
func canonicalize0(frame *rtdata.Frame) {
...
//对应 public native int getBooleanAttributes0(File f);
func getBooleanAttributes0(frame *rtdata.Frame) {
	vars := frame.LocalVars()
	f := vars.GetRef(1)
	path := _getPath(f)
